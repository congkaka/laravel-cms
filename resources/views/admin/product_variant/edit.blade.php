        ...
        <div class="post d-flex flex-column-fluid" id="kt_post">
            ...
            <div id="kt_content_container" class="container-xxl">
                <form id="kt_ecommerce_add_product_variant_form" method="post"
                      action="{{route('admin.product_variant.update',$item->id)}}" class="form d-flex flex-column flex-lg-row gap-7 gap-lg-10" method="POST">
                    @method('PUT')
                    @csrf
                    <!--begin::Main column-->
                                ...
                                <div class="mb-10 fv-row">
                                    ...
                                    <select name="product_id" required class="form-select mb-2" data-control="select2"
                                        ...
                                        @foreach($products as $p)
                                            <option
                                                {{$item->product_variant_id == $p['id'] ? 'selected' : ''}} value="{{$p['id']}}">{{$p['name']}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="mb-10 fv-row">
                                ...
                                <div class="mb-10 fv-row">
                                    ...
                                    <div class="form-check form-switch">
                                        <input required class="form-check-input" value="1" name="is_active" type="checkbox"
                                               {{$item->is_active ? 'checked' : ''}} role="switch" id="flexSwitchCheckChecked">
                                    </div>
                                </div>
                                <!--end::Card header-->
                            </div>
